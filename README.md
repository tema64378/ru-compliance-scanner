# 🇷🇺 RU Compliance Scanner

![Python](https://img.shields.io/badge/Python-3.9%2B-3776AB?logo=python&logoColor=white)
![License: MIT](https://img.shields.io/badge/License-MIT-22c55e)
![152-FZ](https://img.shields.io/badge/152--ФЗ-compliance-0077FF)
![FastAPI](https://img.shields.io/badge/API-FastAPI-009688?logo=fastapi&logoColor=white)
![Scope](https://img.shields.io/badge/scope-informational-7c3aed)

Проверяет сайт на соответствие требованиям законодательства РФ (прежде всего
**152-ФЗ «О персональных данных»**), показывает что не так, как это исправить и
**какой штраф** грозит, если не починить.

> ⚠️ **Дисклеймер:** инструмент **информационный и эвристический**, это **не
> юридическая консультация**. Суммы штрафов — по КоАП РФ (ст. 13.11, ред. с
> 30.05.2025), приведены для юрлиц. Точную квалификацию определяет Роскомнадзор/суд.
> Проверяй свои или разрешённые сайты.

## Демо

![Пример отчёта](assets/demo.svg)

## Как работает

```mermaid
flowchart LR
    A[URL] --> B[scanner.scan]
    B --> C[checks.py — детект на странице]
    C --> C1[Формы / сбор ПДн?]
    C --> C2[Ссылка на политику]
    C --> C3[Согласие в формах]
    C --> C4[Cookie-баннер]
    C --> C5[HTTPS]
    C1 --> D[rules.py — закон + штраф]
    C2 --> D
    C3 --> D
    C4 --> D
    C5 --> D
    D --> E[Суммарный риск штрафа]
    E --> F[report.py — отчёт простым языком]
```

## Что проверяет

| Требование | Закон | Штраф (юрлица) |
|------------|-------|----------------|
| **Политика обработки ПДн** (ссылка в футере) | ч. 3 ст. 13.11 КоАП | 30 000 – 60 000 ₽ |
| **Согласие в формах** (отдельный непредзаполненный чекбокс) | ч. 2 ст. 13.11 КоАП | 300 000 – 500 000 ₽ |
| **Cookie-баннер** с активным согласием | ч. 1 ст. 13.11 КоАП | 150 000 – 300 000 ₽ |
| **HTTPS** на страницах с формами | ст. 19 152-ФЗ | 150 000 – 300 000 ₽ |
| **Редирект http → https** (если есть формы) | ст. 19 152-ФЗ | 150 000 – 300 000 ₽ |
| **Уведомление РКН** о начале обработки | ч. 10 ст. 13.11 КоАП | 100 000 – 300 000 ₽ *(проверь сам)* |
| **Возрастная маркировка** (0+ … 18+) | ст. 6.17 КоАП; 436-ФЗ | 20 000 – 50 000 ₽ *(если применимо)* |
| **Маркировка рекламы** (ERID / ОРД) | ст. 14.3 КоАП; ФЗ-38 | 100 000 – 500 000 ₽ *(если применимо)* |
| **Реквизиты продавца** (магазины) | ст. 14.8 КоАП; ЗоЗПП | 5 000 – 10 000 ₽ |
| **Онлайн-касса / чек** (приём оплаты) | ст. 14.5 КоАП; 54-ФЗ | от 30 000 ₽ *(проверь сам)* |

Каждое нарушение сопровождается **прямой ссылкой на статью КоАП в КонсультантПлюс**.
Сканер также определяет признаки **интернет-магазина / приёма оплаты** и включает
соответствующие требования (ЗоЗПП, 54-ФЗ).

Сканер сам определяет, **собирает ли сайт ПДн** (есть ли формы), и применяет
требования по контексту. Он также:
- **находит трекеры/аналитику** (Яндекс.Метрика, GA/GTM, VK-пиксель и др.) и реально
  выставленные cookie — именно они рождают обязанность cookie-согласия;
- **политику проверяет «по-взрослому»** — заходит по ссылке (и пробует типовые пути
  `/privacy`, `/policy`…), смотрит, открывается ли и есть ли обязательные блоки;
- с флагом `--render` **рендерит страницу через headless Chrome** и видит контент,
  который подгружается JS (баннеры, формы, скрипты) — точнее и стабильнее.

В конце считает **суммарный потенциальный штраф**.

## Установка

```bash
cd ru-compliance-scanner
python3 -m venv .venv && source .venv/bin/activate
pip install -r requirements.txt
```

## Использование

```bash
python cli.py                      # интерактивно: вводи адреса по одному
python cli.py example.ru           # разбор простым языком + штрафы
python cli.py example.ru --json
python cli.py example.ru --html    # HTML-заключение (report_<host>.html)
python cli.py example.ru --pdf     # PDF-заключение (через Chrome)
python cli.py example.ru --render  # рендер через headless Chrome (видит JS-контент)

# Batch — несколько сайтов разом + сводная таблица рисков
python cli.py a.ru b.ru c.ru
python cli.py --file sites.txt

# API
uvicorn api.main:app
curl "localhost:8000/scan?url=example.ru"
```

### Telegram-бот

Кидаешь боту ссылку → он присылает нарушения со штрафами.

```bash
export BOT_TOKEN=...   # токен от @BotFather
python bot.py
```

## 💸 Генератор документов

Скан нашёл, что чего-то не хватает — сгенерируй готовые документы под свой сайт
(политика обработки ПДн, согласие для форм, cookie-политика) в `.md`, `.html` и `.pdf`:

```bash
python gen.py                    # интерактивно: пара вопросов
python gen.py --scan example.ru  # предзаполнить сайт/аналитику из скана
```

> Документы — шаблоны на основе типовой структуры 152-ФЗ. Проверьте и адаптируйте
> под себя; это не заменяет юридическую консультацию.

## 💳 Приём оплаты (Робокасса)

Лендинг (`landing/index.html`) → форма заказа → `POST /order` → ссылка на оплату
Робокассы → после оплаты колбэк `/robokassa/result` помечает заказ оплаченным →
страница `/robokassa/success` отдаёт документы.

Ключи из личного кабинета Робокассы — в переменные окружения:
```bash
export ROBOKASSA_LOGIN=...   # идентификатор магазина
export ROBOKASSA_PASS1=...   # пароль #1 (формирование платежа)
export ROBOKASSA_PASS2=...   # пароль #2 (проверка колбэка)
export ROBOKASSA_TEST=1      # тестовый режим (убрать на проде)
uvicorn api.main:app
```
В кабинете Робокассы укажи: **Result URL** → `https://домен/robokassa/result`,
**Success URL** → `/robokassa/success`, **Fail URL** → `/robokassa/fail`.
А в `landing/index.html` впиши адрес бэкенда в `const API_BASE`.

> 💡 Для приёма денег оформи **самозанятость** («Мой налог») — чек формируется в
> приложении, онлайн-касса не нужна. Робокасса работает с самозанятыми.

## Структура

| Путь | Роль |
|------|------|
| `rucompliance/rules.py` | справочник требований + суммы штрафов (КоАП) |
| `rucompliance/checks.py` | эвристический детект признаков на странице (BeautifulSoup) |
| `rucompliance/scanner.py` | оркестратор + подсчёт риска штрафа |
| `rucompliance/report.py` | вывод простым языком (rich) |
| `rucompliance/html_report.py` | HTML/PDF-заключение |
| `rucompliance/docgen.py` | шаблоны и генератор документов 152-ФЗ |
| `rucompliance/cookiegen.py` | генератор готового cookie-баннера |
| `rucompliance/badge.py` | SVG-бейдж «проверено на 152-ФЗ» |
| `payments/robokassa.py` / `payments/email_send.py` | оплата Робокассы + отправка документов на e-mail |
| `api/pay.py` | поток покупки: заказ → оплата → выдача документов |
| `monitor.py` | мониторинг сайтов + алерт в Telegram при новом нарушении |
| `landing/index.html` | лендинг сервиса (видео-hero, маскот, калькулятор штрафа) |
| `cli.py` / `api/main.py` / `bot.py` / `gen.py` | CLI, API, Telegram-бот, генератор документов |

## Возможности сервиса

- 🕸️ **Многостраничный обход** — сканирует ключевые страницы (контакты, оплата, политика), а не только главную.
- 🔗 **Ссылки на статьи КоАП** в КонсультантПлюс под каждым нарушением.
- 🍪 **Генератор cookie-баннера** — `GET /cookie-banner` или в пакете документов (закрывает самое частое нарушение).
- 🔁 **Мониторинг** (`monitor.py`) — перепроверка по расписанию + алерт в Telegram.
- 📧 **E-mail-доставка** документов после оплаты (SMTP).
- 🏅 **Бейдж** — `GET /badge?url=...` (встраивается на сайт клиента).
- 🎣 **Лид-ген CSV** — `python cli.py a.ru b.ru --csv leads.csv`.

## Дорожная карта

- [x] HTML / PDF-заключение, брендированное
- [x] Многостраничный обход + ссылки на КоАП (КонсультантПлюс)
- [x] Генератор cookie-баннера, мониторинг, e-mail, бейдж, лид-ген CSV
- [x] Расширенная база: 152-ФЗ, реклама (14.3), ЗоЗПП (14.8), 436-ФЗ (6.17), 54-ФЗ (14.5)
- [ ] AI-слой (Claude API): умный анализ страницы и адаптивная генерация документов
- [ ] Учёт ИП/должностных лиц в расчёте штрафа
